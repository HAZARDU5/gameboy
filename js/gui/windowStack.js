...
export default class WindowStack {
    ...
    registerMouseEvents() {
        if (!this.hookedMouse) {
            ...
            for (eventIndex in this.events) {
                dom.addEvent(this.events[eventIndex][0], document, this.events[eventIndex][1]);
            }
            this.center();
        }
    };
    ...
    unregisterMouseEvents() {
        if (this.hookedMouse) {
            ...
            var eventIndex;
            for (eventIndex in this.events) {
                dom.removeEvent(this.events[eventIndex][0], document, this.events[eventIndex][1]);
            }
        }
    };
    ...
    getStyleFloatOf(propertyNameOf, JSpropertyNameOf) {
        ...
        try {
            ...
            try {
                dirtyValue = this.domObject.currentStyle.getAttribute(JSpropertyNameOf);
                /*JS object notation style keywords, not the CSS keywords!*/
            }
            catch (error) {
                terminal.cout(error.message, 2);
            }
        }
        var cleanValue = parseFloat((dirtyValue != null && dirtyValue.length) ? dirtyValue : "0");
        return (!isNaN(cleanValue) && cleanValue != -1) ? cleanValue : 0;
        /*Note: Opera gives -1, not NaN for cleanValue when no number is found.*/
    };
    ...
    dragEnable(event) {
        ...
        var mousedWindow = dom.isSameNode(elementAt, this.domObject);
        this.mousedInsideWindow = (mousedWindow) ? false : dom.isDescendantOf(this.domObject, elementAt);
        if (mousedWindow || this.mousedInsideWindow) {
            ...
            this.resize = !!(highX - this.leftLast < 25 && highY - this.topLast < 25);
    ...
    interceptOpacity() {
        ...
        return (sampledOpacity > 0) ? sampledOpacity : 100;
        /*Dirty opacity first-set for IE*/
    };
